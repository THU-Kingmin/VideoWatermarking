...
from video_chatgpt.inference_flow_mask import video_chatgpt_infer, video_chatgpt_attack_flow_tgt
import torch
import cv2
import numpy as np
...
def parse_args():
    ...
    parser.add_argument("--weight_spa", type=float, default=0.5)
...
def run_inference(args):
    ...
    for sample in tqdm(gt_contents):
        ...
        question = sample['Q']
        answer = sample['A']
        video_name_tgt = sample['video_name_tgt']
        question_tgt = sample['Q_tgt']
        answer_tgt = sample['A_tgt']
        ...
        for fmt in video_formats:  # Added this line
            ...
            if os.path.exists(temp_path):
                ...
                break
        for fmt in video_formats:  # Added this line
            temp_path_tgt = os.path.join(args.video_dir, f"{video_name_tgt}{fmt}")
            if os.path.exists(temp_path_tgt):
                video_path_tgt = temp_path_tgt
                break

        # Check if the video exists
        if video_path is not None:  # Modified this line
            video_frames = load_video(video_path, num_frm=args.seq_len)
        if video_path_tgt is not None:  # Modified this line
            video_frames_tgt = load_video(video_path_tgt, num_frm=args.seq_len)
        ...
        output_tgt = video_chatgpt_infer(video_frames_tgt, question, conv_mode, model, vision_tower,
                                            tokenizer, image_processor, video_token_len)
        output_list_txt_tgt.append(output_tgt)


        
        if args.mask_type == 'flow':
            output_attack,image_clean, image_attack, delta_attack, modifier, output_vis, output_clip = video_chatgpt_attack_flow_tgt(video_frames, video_frames_tgt, flow_sort, question, question, conv_mode, model, vision_tower,
        ...
        question_list_txt_tgt.append(question_tgt)
        print('question: ', question)
        print('answer: ', answer)
        print('question_tgt: ', question_tgt)
        print('answer_tgt: ', answer_tgt)
        
        print('clean: ', output)
        print('tgt: ', output_tgt)
    ...
    with open(os.path.join(args.output_dir, save_path, f"clean.txt"), 'w', encoding='utf-8') as f1:
        for text in output_list_txt:
            f1.write(text + '\n')
    with open(os.path.join(args.output_dir, save_path, f"tgt.txt"), 'w', encoding='utf-8') as f7:
        for text in output_list_txt_tgt:
            f7.write(text + '\n')
    with open(os.path.join(args.output_dir, save_path, f"answer.txt"), 'w', encoding='utf-8') as f2:
        for text in answer_list_txt:
            f2.write(text + '\n')
    with open(os.path.join(args.output_dir, save_path, f"answer_tgt.txt"), 'w', encoding='utf-8') as f5:
        for text in answer_list_txt_tgt:
            f5.write(text + '\n')
    with open(os.path.join(args.output_dir, save_path, f"question.txt"), 'w', encoding='utf-8') as f3:
        for text in question_list_txt:
            f3.write(text + '\n')
    with open(os.path.join(args.output_dir, save_path, f"question_tgt.txt"), 'w', encoding='utf-8') as f6:
        for text in question_list_txt_tgt:
            f6.write(text + '\n')
