...
def video_chatgpt_attack(video_frames, question, conv_mode, model, vision_tower, tokenizer, image_processor, video_token_len, args):
    if model.get_model().vision_config.use_vid_start_end:
        qs = question + '\n' + DEFAULT_VID_START_TOKEN + DEFAULT_VIDEO_PATCH_TOKEN * video_token_len + DEFAULT_VID_END_TOKEN
    else:
        qs = question + '\n' + DEFAULT_VIDEO_PATCH_TOKEN * video_token_len
    ...
    mask = torch.arange(args.seq_len) < (args.seq_len - mask_len)
    print('mask_len:', mask_len, ', seq_len', args.seq_len , mask)
    mask = mask.view(-1, 1, 1, 1).expand_as(modifier).to('cuda')
    # indicator = [1 for _ in range(args.seq_len)] #Frames to be perturbed
    
    # train
    start_time = time.time()
    modifier_leaf = modifier.clone().detach().requires_grad_(True)
    for iter in range(args.num_iter):
        modifier_leaf.data = torch.where(mask, modifier, torch.zeros_like(modifier)) 
        image_attack = torch.clamp((modifier_leaf + image_clean), min_in, max_in)
        ...
        # else:
        loss_spa = torch.sum(torch.sqrt(torch.mean(torch.pow((torch.unsqueeze(modifier_leaf, 0)), 2), dim=0).mean(dim=2).mean(dim=2).mean(dim=1)))
        loss = args.weight_clip * loss_clip + args.weight_llm * loss_llm + args.weight_spa * loss_spa

        loss.backward()
        grad_sign = torch.sign(modifier_leaf.grad)
        modifier_leaf.data -= grad_sign * step
        modifier_leaf.grad.zero_()
        modifier_leaf.data = torch.clamp(modifier_leaf.data, -modif_max, modif_max)

        modifier.data = modifier_leaf.data
