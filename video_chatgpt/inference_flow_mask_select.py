...
def video_chatgpt_attack_flow_select(video_frames, question, conv_mode, model, vision_tower, tokenizer, image_processor, video_token_len, args):
    if model.get_model().vision_config.use_vid_start_end:
        qs = question + '\n' + DEFAULT_VID_START_TOKEN + DEFAULT_VIDEO_PATCH_TOKEN * video_token_len + DEFAULT_VID_END_TOKEN
    else:
        qs = question + '\n' + DEFAULT_VIDEO_PATCH_TOKEN * video_token_len
    ...
    for iter in range(args.num_iter):
        image_attack = torch.clamp((modifier + image_clean), min_in, max_in)
    ...
    new_len = args.seq_len-mask_len
    indices = torch.randperm(32)[:new_len]
    indices = indices.to('cuda')
    image_select = torch.index_select(image_attack, 0, indices)
    print('new_len', new_len, ', indices:', indices)

    image_forward_outs_attack = vision_tower(image_select, output_hidden_states=True)
    frame_features_attack = image_forward_outs_attack.hidden_states[-2][:, 1:]
    video_spatio_temporal_features_attack = get_spatio_temporal_features_torch(frame_features_attack)
    with torch.inference_mode():
    ...
    return outputs_attack, image_clean, image_attack, abs(image_attack-image_clean).mean() * 0.26130258 * 255, (modifier).detach().cpu()
