...
def video_chatgpt_attack_flow(video_frames, flow_sort, question, conv_mode, model, vision_tower, tokenizer, image_processor, video_token_len, args):
    if model.get_model().vision_config.use_vid_start_end:
        qs = question + '\n' + DEFAULT_VID_START_TOKEN + DEFAULT_VIDEO_PATCH_TOKEN * video_token_len + DEFAULT_VID_END_TOKEN
    else:
        qs = question + '\n' + DEFAULT_VIDEO_PATCH_TOKEN * video_token_len
    ...
    for iter in range(args.num_iter):
        ...
        if args.type == 'type1':
            loss = args.weight_clip * loss_clip + args.weight_spa * loss_spa
        elif args.type == 'type2':
            loss = args.weight_llm * loss_llm + args.weight_spa * loss_spa
        else:
    ...
    with torch.inference_mode():
        ...
        output_vis = model.attack(input_ids,
            video_spatio_temporal_features=video_spatio_temporal_features_attack.unsqueeze(0)).logits
        last_time_step_output_vis = output_vis[:, -1, :]
        last_time_step_output_vis = last_time_step_output_vis.cpu()

        output_clip = video_spatio_temporal_features_attack.cpu()
    ...
    return outputs_attack, image_clean, image_attack, abs(image_attack-image_clean).mean() * 0.26130258 * 255, (modifier * mask).detach().cpu(), last_time_step_output_vis, output_clip

def video_chatgpt_attack_flow_tgt(video_frames, video_frames_tgt, flow_sort, question, question_tgt, conv_mode, model, vision_tower, tokenizer, image_processor, video_token_len, args):
    if model.get_model().vision_config.use_vid_start_end:
        qs = question + '\n' + DEFAULT_VID_START_TOKEN + DEFAULT_VIDEO_PATCH_TOKEN * video_token_len + DEFAULT_VID_END_TOKEN
    else:
        qs = question + '\n' + DEFAULT_VIDEO_PATCH_TOKEN * video_token_len
    ...
    conv_tgt = conv_templates[conv_mode].copy()
    conv_tgt.append_message(conv_tgt.roles[0], qs_tgt)
    conv_tgt.append_message(conv_tgt.roles[1], None)
    prompt_tgt = conv_tgt.get_prompt()
    inputs_tgt = tokenizer([prompt_tgt])

    #video_frames 0～255
    print('video_frames size: ', video_frames[0].size)
    image_clean = image_processor.preprocess(video_frames, return_tensors='pt')['pixel_values']
    image_clean = image_clean.half().cuda() #[100, 3, 224, 224]
    print('video_frames tgt size: ', video_frames_tgt[0].size)
    image_tgt = image_processor.preprocess(video_frames_tgt, return_tensors='pt')['pixel_values']
    image_tgt = image_tgt.half().cuda() #[100, 3, 224, 224]

    with torch.no_grad():
        image_forward_outs_tgt = vision_tower(image_tgt, output_hidden_states=True)
        frame_features_tgt = image_forward_outs_tgt.hidden_states[-2][:, 1:] # Use second to last layer as in LLaVA
        video_spatio_temporal_features_tgt = get_spatio_temporal_features_torch(frame_features_tgt)

    input_ids = torch.as_tensor(inputs.input_ids).cuda()
    stop_str = conv.sep if conv.sep_style != SeparatorStyle.TWO else conv.sep2
    stopping_criteria = KeywordsStoppingCriteria([stop_str], tokenizer, input_ids)

    input_ids_tgt = torch.as_tensor(inputs_tgt.input_ids).cuda()
    stop_str_tgt = conv_tgt.sep if conv_tgt.sep_style != SeparatorStyle.TWO else conv_tgt.sep2
    stopping_criteria_tgt = KeywordsStoppingCriteria([stop_str_tgt], tokenizer, input_ids_tgt)

    with torch.no_grad():
        output_tgt = model.attack(input_ids_tgt,
            video_spatio_temporal_features=video_spatio_temporal_features_tgt.unsqueeze(0)).logits
    ...
    for iter in range(args.num_iter):
        ...
        loss_clip = mse_loss(video_spatio_temporal_features_tgt, video_spatio_temporal_features_attack)
        
        loss_llm = mse_loss(output_tgt, output_attack)
