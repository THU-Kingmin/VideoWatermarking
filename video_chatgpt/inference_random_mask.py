...
def video_chatgpt_attack_random(video_frames, question, conv_mode, model, vision_tower, tokenizer, image_processor, video_token_len, args):
    if model.get_model().vision_config.use_vid_start_end:
        qs = question + '\n' + DEFAULT_VID_START_TOKEN + DEFAULT_VIDEO_PATCH_TOKEN * video_token_len + DEFAULT_VID_END_TOKEN
    else:
        qs = question + '\n' + DEFAULT_VIDEO_PATCH_TOKEN * video_token_len
    ...
    random_indices = torch.randperm(args.seq_len)[:args.seq_len-mask_len]
    mask[random_indices] = 1
    print('mask_len:', mask_len, ', seq_len', args.seq_len, random_indices)
